...
-- requires: function-round_control@v1.0
--
-- Rework: no COPY to a server file (needed superuser and a writable path),
-- DISABLE TRIGGER USER instead of ALL (ALL needs superuser), and the deletes
-- run in foreign-key order because FK triggers now stay active.
...
BEGIN
	...
	--Delete everything, children before parents
