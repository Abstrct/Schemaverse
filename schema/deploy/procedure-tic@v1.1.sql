...
-- requires: function-round_control
-- requires: function-move_ships
-- requires: function-perform_mining
-- requires: matview-player_stats
--
-- The game physics that tic.pl used to sequence from Perl. Two procedures own
-- the phases and commit between them; the Go runner in tic/ calls tic_open(),
-- runs every fleet script as its player, then calls tic_close().
--
-- The per-phase work lives in plain functions so it can be tested inside a
-- transaction (procedures that COMMIT cannot be called from one). The
-- procedures carry no SET clause: a procedure with one cannot COMMIT.
